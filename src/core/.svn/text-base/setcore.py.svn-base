...
def update_set():
    print "\n\n"
    print_error("The Social-Engineer Toolkit has officially moved to github and no longer uses SVN.")
    print_error("Ensure that you have GIT installed and this conversion tool will automatically pull the latest git version for you.")
    print_error("Do you want to do a manual install or have SET do the conversion to GIT for you?")
    question1 = raw_input("\n1. Automatic\n2. Manual\n3. Continue using SET (NO UPDATES ANYMORE!)\n\nEnter your numeric choice: ")
    if question1 == "1" or question1 == "":
	print_status("BEFORE YOU START! Ensure you have GIT installed (apt-get install git)")
	confirm = raw_input("Have you installed GIT? y/n: ")
	if confirm == "n" or confirm == "no": question1 = "2"
	if confirm == "y" or confirm == "yes":
		print_status("Great! Here we go... Removing old svn repository and moving to new")
		set_dir = os.getcwd()
		os.chdir("../")
		if os.path.isfile(set_dir + "/set"):
			shutil.rmtree(set_dir)
			print_status("SET directory has been removed. Now checking out SET from GIT..")
			print_status("This could take a few moments..")
			subprocess.Popen("git clone https://github.com/trustedsec/social-engineer-toolkit " + set_dir, shell=True).wait()
			os.chdir(set_dir)
			print_status("You should now have the latest from git. To update, run set-update or type git pull")
			print_status("Exit SET and restart. Move out of the current directory and go into the set directory.")
			print_status("You should never have to go through this process again!")
			sys.exit()

    if question1 == "2":
		print_status("Manual instructions:\n\nRemove the SET directory (rm -rf set/) then type:\n\ngit clone https://github.com/trustedsec/social-engineer-toolkit set/")
		pause = raw_input("Press {return} to continue to use SET.")
